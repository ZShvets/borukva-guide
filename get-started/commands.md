...
Для зручності на сервері є команда `/polydex` для перегляду всіх ванільних/модових рецептів

Для більш конкретних запитів (Також доступні в інтерфейсі команди)

`/polydex category` [категорія]

`/polydex entry_result` [назва предмету] - з чого отримується

`/polydex entry_usage` [назва предмету] - для чого використовується

`/polydex search` [букви/слово] - пошук по назві
